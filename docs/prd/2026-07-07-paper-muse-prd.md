...
**persona 原文槽位**：「第一性原理」「对抗式审查」两组提示词原文待用户提供，现为要旨转述版。
...
每里程碑完成即真实使用一轮，北极星（质量地板）持续观测。M1/M2 顺序可互换；M3 依赖 M0 通过与「对抗式审查」persona 原文到位。
...
- **persona 原文**：两组提示词原文待用户提供（阻塞 R4 persona 与扫描第一性拆解的最终形态）。
