...
每主张证伪检索（#8 = gpt-researcher 多源）→ 作者代理答辩 → 仲裁 meta-review →
逐失败点三态裁决（已证伪 / 有佐证 / 未决）→ 流式 → 落 failure-points.md。
...
AUTHOR_STANCES = ["认", "驳", "存疑"]       # 作者代理答辩立场
META_DECISIONS = ["维持", "缓和", "加重"]   # 仲裁对红队失败点的处理建议（不改代码裁决）
...
def author_rebuttal(claim_text: str, failure: dict, evidence: list, llm_call) -> dict:
    """作者代理答辩：只负责回应红队失败点，不拥有裁决权。"""
    prompt = (
        "你现在扮演论文作者的答辩代理。只基于中心主张、红队失败点和已检证据回应，"
        "不能编造不存在的文献；证据为空时必须承认仍需补证。\n\n"
        f"中心主张：{claim_text}\n"
        f"红队失败点：{failure.get('statement', '')}\n"
        f"现有证据：\n{_evidence_brief(evidence)}\n\n"
        "给出作者方答辩：\n"
        "- stance：只能是 认 / 驳 / 存疑；\n"
        "- argument：一段短答辩；\n"
        "- needed_evidence：若仍需补证，说明需要哪类证据；否则可为空。\n"
        '只输出 JSON：{"stance": "认|驳|存疑", "argument": "...", "needed_evidence": "..."}'
    )
    try:
        raw = extract_json(llm_call(prompt))
    except Exception as e:
        return {"stance": "存疑", "argument": f"作者代理答辩失败：{e}", "needed_evidence": ""}
    stance = str(raw.get("stance", "")).strip()
    return {
        "stance": stance if stance in AUTHOR_STANCES else "存疑",
        "argument": re.sub(r"\s+", " ", str(raw.get("argument", ""))).strip(),
        "needed_evidence": re.sub(r"\s+", " ", str(raw.get("needed_evidence", ""))).strip(),
    }


def meta_review(claim_text: str, failure: dict, evidence: list, rebuttal: dict,
                verdict: str, llm_call) -> dict:
    """仲裁 meta-review：解释红队/作者/证据之间的关系，但不得覆盖代码裁决。"""
    prompt = (
        "你是仲裁 meta-reviewer。你要比较红队失败点、作者答辩和已检证据，给出编辑性裁决理由。"
        "最终裁决已经由代码按证据三态锁定，不能被你改写；没有直接证据时必须维持未决、不放行。\n\n"
        f"中心主张：{claim_text}\n"
        f"红队失败点：{failure.get('statement', '')}\n"
        f"代码裁决：{verdict}\n"
        f"作者答辩：{(rebuttal or {}).get('stance', '存疑')}；{(rebuttal or {}).get('argument', '')}\n"
        f"现有证据：\n{_evidence_brief(evidence)}\n\n"
        "输出：\n"
        "- decision：只能是 维持 / 缓和 / 加重；\n"
        "- reason：为什么这样处理；\n"
        "- revision：作者下一版应如何改写或补证。\n"
        '只输出 JSON：{"decision": "维持|缓和|加重", "reason": "...", "revision": "..."}'
    )
    try:
        raw = extract_json(llm_call(prompt))
    except Exception as e:
        raw = {"decision": "维持", "reason": f"仲裁失败：{e}", "revision": ""}
    decision = str(raw.get("decision", "")).strip()
    if verdict == "未决" and not evidence:
        decision = "维持"
    elif decision not in META_DECISIONS:
        decision = "维持"
    reason = re.sub(r"\s+", " ", str(raw.get("reason", ""))).strip()
    if not reason:
        reason = f"按已检证据维持代码裁决：{verdict}"
    return {
        "decision": decision,
        "reason": reason,
        "revision": re.sub(r"\s+", " ", str(raw.get("revision", ""))).strip(),
...
def _apply_falsify_pool(claim, pool, classify_llm, author_llm=None, meta_llm=None,
    ...
    for f in claim["failures"]:
        ...
        if author_llm:
            f["author_rebuttal"] = author_rebuttal(claim["text"], f, evidence, author_llm)
        if meta_llm:
            f["meta_review"] = meta_review(
                claim["text"], f, evidence, f.get("author_rebuttal") or {}, f["verdict"], meta_llm
            )
    return claim
...
def _write_failure_points(output_dir, claims):
    ...
    for claim in claims:
        ...
        for f in claim.get("failures", []):
            ...
            if f.get("author_rebuttal"):
                rb = f["author_rebuttal"]
                lines.append(f"- 作者答辩：{rb.get('stance', '存疑')}｜{rb.get('argument', '')}")
                if rb.get("needed_evidence"):
                    lines.append(f"- 作者补证需求：{rb['needed_evidence']}")
            if f.get("meta_review"):
                mr = f["meta_review"]
                lines.append(f"- 仲裁：{mr.get('decision', '维持')}｜{mr.get('reason', '')}")
                if mr.get("revision"):
                    lines.append(f"- 修订建议：{mr['revision']}")
            if v == "未决":
...
def run_review(source_text, has_draft, output_dir, review_llm, falsify_search,
    ...
    author_llm / meta_llm 为空时跳过对应阶段（离线单测可只测核心证据管线）；
    真实服务端路径会传强模型启用两阶段。
